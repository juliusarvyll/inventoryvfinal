...
class ComponentImporter extends Importer
{
        ...
        return [
            Select::make('default_category_id')
                ...
                ->options(fn (): array => Category::query()
                    ->ofType(InventoryCategoryType::Component)
                    ->orderBy('name')
                    ->pluck('name', 'id')
                    ->all())
                ->searchable()
    ...
    protected function beforeSave(): void
    {
        ...
        if (
            ...
            && tap(Component::query()->where('serial', $this->record->serial), function ($query): void {
                if ($this->record->exists) {
                    $query->whereKeyNot($this->record->getKey());
                }
            })->exists()
        ) {
            throw new RowImportFailedException("The serial [{$this->record->serial}] is already assigned to another component.");
        }
    }
